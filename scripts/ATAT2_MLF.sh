...
if [ ${niftyReg} = "true" ]; then
    ...
    if [ ! -f ${mask_dir}/left_MLF_mask_dil.nii.gz ]; then
        reg_resample -flo ${anatomical_priors}/left_MLF_mask_dil.nii.gz -ref ${T1} -trans ${base_roi_dir}/niftiReg/MNI_to_T1_cpp.nii.gz -res ${mask_dir}/left_MLF_mask_dil.nii.gz
    fi
    if [ ! -f ${mask_dir}/right_MLF_mask_dil.nii.gz ]; then
        reg_resample -flo ${anatomical_priors}/right_MLF_mask_dil.nii.gz -ref ${T1} -trans ${base_roi_dir}/niftiReg/MNI_to_T1_cpp.nii.gz -res ${mask_dir}/right_MLF_mask_dil.nii.gz
    fi
else
    ...
    if [ ! -f ${mask_dir}/left_MLF_mask_dil.nii.gz ]; then
        mri_easywarp --i ${anatomical_priors}/left_MLF_mask_dil.nii.gz --o ${mask_dir}/left_MLF_mask_dil.nii.gz --field ${base_roi_dir}/easyReg/MNI_to_T1_fwd.nii.gz --threads ${threads}
    fi
    if [ ! -f ${mask_dir}/right_MLF_mask_dil.nii.gz ]; then
        mri_easywarp --i ${anatomical_priors}/right_MLF_mask_dil.nii.gz --o ${mask_dir}/right_MLF_mask_dil.nii.gz --field ${base_roi_dir}/easyReg/MNI_to_T1_fwd.nii.gz --threads ${threads}
    fi
fi

echo "Extracting the cortical ROIs from the GIF parcellation"
#SEEDS and WAYPOINTS
if [ ! -f ${roi_dir}/left_MLF_seed.nii.gz ]; then
    mrcalc ${gif_parc} 204 -eq ${roi_dir}/left_MLF_TP.nii.gz
    mrcalc ${gif_parc} 202 -eq ${roi_dir}/left_MLF_STG.nii.gz
    mrcalc ${roi_dir}/left_MLF_TP.nii.gz ${roi_dir}/left_MLF_STG.nii.gz -add ${roi_dir}/left_MLF_seed.nii.gz -force
fi
if [ ! -f ${roi_dir}/left_MLF_termination.nii.gz ]; then
    mrcalc ${gif_parc} 200 -eq ${roi_dir}/left_MLF_SPL.nii.gz
    mrcalc ${gif_parc} 170 -eq ${roi_dir}/left_MLF_preCun.nii.gz
    mrcalc ${roi_dir}/left_MLF_preCun.nii.gz ${roi_dir}/left_MLF_SPL.nii.gz -add ${roi_dir}/left_MLF_termination.nii.gz -force
fi
if [ ! -f ${roi_dir}/right_MLF_seed.nii.gz ]; then
    mrcalc ${gif_parc} 203 -eq ${roi_dir}/right_MLF_TP.nii.gz
    mrcalc ${gif_parc} 201 -eq ${roi_dir}/right_MLF_STG.nii.gz
    mrcalc ${roi_dir}/right_MLF_TP.nii.gz ${roi_dir}/right_MLF_STG.nii.gz -add ${roi_dir}/right_MLF_seed.nii.gz -force
fi
if [ ! -f ${roi_dir}/right_MLF_termination.nii.gz ]; then
    mrcalc ${gif_parc} 199 -eq ${roi_dir}/right_MLF_SPL.nii.gz
    mrcalc ${gif_parc} 169 -eq ${roi_dir}/right_MLF_preCun.nii.gz
    mrcalc ${roi_dir}/right_MLF_preCun.nii.gz ${roi_dir}/right_MLF_SPL.nii.gz -add ${roi_dir}/right_MLF_termination.nii.gz -force
fi

echo "Dilating the cortical ROIs into the 5tt white matter and 7T mask"
#Dilation
if [ ! -f ${roi_dir}/left_MLF_seed_dil.nii.gz ]; then
    ${dilation} --hsvs_5tt ${fivett} --mask ${mask_dir}/left_MLF_mask_dil.nii.gz --in ${roi_dir}/left_MLF_seed.nii.gz --out ${roi_dir}/left_MLF_seed_dil.nii.gz
fi
if [ ! -f ${roi_dir}/left_MLF_termination_dil.nii.gz ]; then
    ${dilation} --hsvs_5tt ${fivett} --mask ${mask_dir}/left_MLF_mask_dil.nii.gz --in ${roi_dir}/left_MLF_termination.nii.gz --out ${roi_dir}/left_MLF_termination_dil.nii.gz
fi
if [ ! -f ${roi_dir}/right_MLF_seed_dil.nii.gz ]; then
    ${dilation} --hsvs_5tt ${fivett} --mask ${mask_dir}/right_MLF_mask_dil.nii.gz --in ${roi_dir}/right_MLF_seed.nii.gz --out ${roi_dir}/right_MLF_seed_dil.nii.gz
fi
if [ ! -f ${roi_dir}/right_MLF_termination_dil.nii.gz ]; then
    ${dilation} --hsvs_5tt ${fivett} --mask ${mask_dir}/right_MLF_mask_dil.nii.gz --in ${roi_dir}/right_MLF_termination.nii.gz --out ${roi_dir}/right_MLF_termination_dil.nii.gz
fi

echo "Performing tractography"
#Tckgen
if [ ! -f ${out_dir}/left_MLF.tck ]; then
    tckgen ${FOD} -algorithm ${algorithm} -act ${fivett} -seed_image ${roi_dir}/left_MLF_seed_dil.nii.gz -include ${roi_dir}/left_MLF_termination_dil.nii.gz -maxlength 300 ${out_dir}/left_MLF_prob_SW.tck -seed_unidirectional ${options} -force -mask ${mask_dir}/left_MLF_mask_dil.nii.gz -seeds 25000000 -select 10k
    tckgen ${FOD} -algorithm ${algorithm} -act ${fivett} -seed_image ${roi_dir}/left_MLF_termination_dil.nii.gz -include ${roi_dir}/left_MLF_seed_dil.nii.gz -maxlength 300 ${out_dir}/left_MLF_prob_WS.tck -seed_unidirectional ${options} -force -mask ${mask_dir}/left_MLF_mask_dil.nii.gz -seeds 25000000 -select 10k
    tckedit ${out_dir}/left_MLF_prob_SW.tck ${out_dir}/left_MLF_prob_WS.tck ${out_dir}/left_MLF_prob_unpruned.tck
    tckedit ${out_dir}/left_MLF_prob_unpruned.tck -include ${roi_dir}/left_MLF_seed_dil.nii.gz -include ${roi_dir}/left_MLF_termination_dil.nii.gz ${out_dir}/left_MLF_prob_unpruned_ends.tck -ends_only
    ${pruning} -in ${out_dir}/left_MLF_prob_unpruned_ends.tck -templ_im ${T1} -out ${out_dir}/left_MLF.tck -thr 0.05
    if [ -f ${out_dir}/left_MLF.tck ]; then
        rm -r ${out_dir}/left_MLF_prob_SW.tck ${out_dir}/left_MLF_prob_WS.tck ${out_dir}/left_MLF_prob_unpruned.tck ${out_dir}/left_MLF_prob_unpruned_ends.tck
    fi
fi
if [ ! -f ${out_dir}/right_MLF.tck ]; then
    tckgen ${FOD} -algorithm ${algorithm} -act ${fivett} -seed_image ${roi_dir}/right_MLF_seed_dil.nii.gz -include ${roi_dir}/right_MLF_termination_dil.nii.gz -maxlength 300 ${out_dir}/right_MLF_prob_SW.tck -seed_unidirectional ${options} -force -mask ${mask_dir}/right_MLF_mask_dil.nii.gz -seeds 25000000 -select 10k
    tckgen ${FOD} -algorithm ${algorithm} -act ${fivett} -seed_image ${roi_dir}/right_MLF_termination_dil.nii.gz -include ${roi_dir}/right_MLF_seed_dil.nii.gz -maxlength 300 ${out_dir}/right_MLF_prob_WS.tck -seed_unidirectional ${options} -force -mask ${mask_dir}/right_MLF_mask_dil.nii.gz -seeds 25000000 -select 10k
    tckedit ${out_dir}/right_MLF_prob_SW.tck ${out_dir}/right_MLF_prob_WS.tck ${out_dir}/right_MLF_prob_unpruned.tck
    tckedit ${out_dir}/right_MLF_prob_unpruned.tck -include ${roi_dir}/right_MLF_seed_dil.nii.gz -include ${roi_dir}/right_MLF_termination_dil.nii.gz ${out_dir}/right_MLF_prob_unpruned_ends.tck -ends_only
    ${pruning} -in ${out_dir}/right_MLF_prob_unpruned_ends.tck -templ_im ${T1} -out ${out_dir}/right_MLF.tck -thr 0.05
    if [ -f ${out_dir}/right_MLF.tck ]; then
        rm -r ${out_dir}/right_MLF_prob_SW.tck ${out_dir}/right_MLF_prob_WS.tck ${out_dir}/right_MLF_prob_unpruned.tck ${out_dir}/right_MLF_prob_unpruned_ends.tck
    fi
fi